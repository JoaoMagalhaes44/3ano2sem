...
void computeNormal(int i, int j, std::vector<float>& normal) {
    // Get the heights of neighboring vertices to compute the normal
    float heightLeft = h2(i - 1, j);
    float heightRight = h2(i + 1, j);
    float heightDown = h2(i, j - 1);
    float heightUp = h2(i, j + 1);

    // Calculate the partial derivatives using secant approximation
    float dx[3] = { 1.0f, heightRight - heightLeft, 0.0f }; // Tangent vector in x-direction
    float dz[3] = { 0.0f, heightUp - heightDown, 1.0f }; // Tangent vector in z-direction

    // Compute the cross product of the tangent vectors to get the normal vector
    float normalVector[3];
    cross(dx, dz, normalVector);

    // Compute the normal using the cross product of the tangent vectors
    normalize(normalVector);

    // Store the computed normal into the vector
    normal.push_back(normalVector[0]);
    normal.push_back(normalVector[1]);
    normal.push_back(normalVector[2]);
}
...
void prepareTerrain() {
    ...
    for (int i = 1; i < imageWidth - 2; i++) {
        for (int j = 1; j < imageWidth - 1; j++) {
            ...
            float u1 = (i) / (imageWidth);
            float v1 = (j) / (imageWidth);
            texCoord.push_back(u1);
            texCoord.push_back(v1);
            ...
            float u2 = static_cast<float>(i - 1) / static_cast<float>(imageWidth);
            float v2 = static_cast<float>(j) / static_cast<float>(imageWidth);
            texCoord.push_back(u2);
            texCoord.push_back(v2);
