...
Condition - IF Options  ELSE Options THEN       ->   if testa a condição da stack e nao o do options,  o options é executado se o if for verificado
          | IF Options  THEN
...
#substitui todas as ocorrências dos padrões (LOOPLABEL), (ELSE) e (ENDIF) no texto original pelo identificador "id" seguido do número "1" + o grupo capturado em questão.
...
# subsituir o I para loopvar e depois é passado para o que tem que sair corretamente na função que é o pushg...
# depois como é bottom up, quando chegar à primeira produção se ainda existir algo loopvar escrito quer dizer que o I foi mal usado então dá erro
def p_s1(p):
    """
    S     : Options 
    """
...
# leva antes o numero de iterações e o numero onde começa
def p_loop1(p):
    """
    Loop        : DO Options LOOP 
    """
...
# a condição do if está antes do if
def p_condition1(p):
    """
...
# as strings já incluem o ponto por isso o ponto so apanha inteiros
def p_io1(p):
    """
    Io           : DOT
    """
